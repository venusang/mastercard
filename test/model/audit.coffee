...
describe 'Model Class:  LoyaltyRtrSdk.models.Audit', ->
  ...
  describe 'fields', ->
    ...
    it 'has a field `action_desc` of type `string`', ->
      fieldDefinition = _.findWhere(modelClass::fieldDefinitions, {name: 'action_desc'})
      assert.isDefined fieldDefinition
      assert.equal fieldDefinition.type, 'string'
    
    
    it 'has a field `after_value` of type `hash`', ->
      fieldDefinition = _.findWhere(modelClass::fieldDefinitions, {name: 'after_value'})
      assert.isDefined fieldDefinition
      assert.equal fieldDefinition.type, 'hash'
    
    
    it 'has a field `before_value` of type `hash`', ->
      fieldDefinition = _.findWhere(modelClass::fieldDefinitions, {name: 'before_value'})
      assert.isDefined fieldDefinition
      assert.equal fieldDefinition.type, 'hash'
    
    
    it 'has a field `field_name` of type `string`', ->
      fieldDefinition = _.findWhere(modelClass::fieldDefinitions, {name: 'field_name'})
      assert.isDefined fieldDefinition
      assert.equal fieldDefinition.type, 'string'
    
    
    it 'has a field `performed_at` of type `time`', ->
      fieldDefinition = _.findWhere(modelClass::fieldDefinitions, {name: 'performed_at'})
      assert.isDefined fieldDefinition
    ...
    it 'has a field `user_id` of type `integer`', ->
      fieldDefinition = _.findWhere(modelClass::fieldDefinitions, {name: 'user_id'})
      assert.isDefined fieldDefinition
      assert.equal fieldDefinition.type, 'integer'
