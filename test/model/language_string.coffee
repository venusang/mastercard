...
describe 'Model Class:  LoyaltyRtrSdk.models.LanguageString', ->
  ...
  describe 'fields', ->
    ...
    it 'has a field `fallback` of type `boolean`', ->
      fieldDefinition = _.findWhere(modelClass::fieldDefinitions, {name: 'fallback'})
      assert.isDefined fieldDefinition
      assert.equal fieldDefinition.type, 'boolean'
    
    
    it 'has a field `global` of type `boolean`', ->
      fieldDefinition = _.findWhere(modelClass::fieldDefinitions, {name: 'global'})
      assert.isDefined fieldDefinition
      assert.equal fieldDefinition.type, 'boolean'
    
    
    it 'has a field `label_id` of type `string`', ->
      fieldDefinition = _.findWhere(modelClass::fieldDefinitions, {name: 'label_id'})
      assert.isDefined fieldDefinition
    ...
    it 'has a field `value` of type `string`', ->
      fieldDefinition = _.findWhere(modelClass::fieldDefinitions, {name: 'value'})
      assert.isDefined fieldDefinition
      assert.equal fieldDefinition.type, 'string'
